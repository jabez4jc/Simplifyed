...
echo "Step 1: Importing instruments data..."
echo ""

# Create import SQL script
cat > /tmp/import_instruments_step1.sql << 'EOF'
...
sqlite3 "$DB_FILE" < /tmp/import_instruments_step1.sql 2>&1
...
echo ""
echo "Step 2: Rebuilding FTS table..."
echo ""

# Create FTS rebuild script
cat > /tmp/import_instruments_step2.sql << 'EOF'
-- Rebuild FTS table
...
-- Get FTS count
SELECT 'Total FTS records: ' || COUNT(*) FROM instruments_fts;
EOF

# Rebuild FTS
sqlite3 "$DB_FILE" < /tmp/import_instruments_step2.sql 2>&1

echo ""
echo "Step 3: Updating refresh log..."
echo ""
...
echo "📊 Instruments by exchange:"
sqlite3 "$DB_FILE" "SELECT exchange, COUNT(*) as count FROM instruments GROUP BY exchange ORDER BY count DESC LIMIT 10;" | column -t -s '|'
echo ""

# Show breakdown by instrument type
echo "📊 Instruments by type:"
sqlite3 "$DB_FILE" "SELECT instrumenttype, COUNT(*) as count FROM instruments GROUP BY instrumenttype ORDER BY count DESC;" | column -t -s '|'
echo ""
...
echo "============================================================"
echo ""
echo "You can now use the fast symbol search feature!"
echo ""

# Cleanup
rm -f /tmp/import_instruments_step1.sql /tmp/import_instruments_step2.sql
